...
git_branch() {
  echo $(/usr/local/git/bin/git symbolic-ref HEAD 2>/dev/null | awk -F/ {'print $NF'})
}

git_dirty() {
  st=$(/usr/local/git/bin/git status 2>/dev/null | tail -n 1)
  if [[ $st == "" ]]
  then
    echo ""
  else
...
rvm_prompt(){
  ...
  then
	  echo "%{$fg_bold[yellow]%}$(rvm tools identifier | sed -e s/ruby-//)%{$reset_color%}"
	else
	  echo ""
  fi
}
